...
module Brick
          ...
          fringe.each do |seed_model|
            ...
            collection&.each do |obj|
              ...
              if klass_name == 'ActiveStorage::Attachment' || klass_name == 'ActionText::EncryptedRichText'
                ...
                seed_row << "#{var_name} = #{klass_name}.find_or_create_by(#{(fk_vals + data).join(', ')}) do |asa|
  ...
  asa.updated_at = DateTime.parse('#{obj.updated_at.inspect}')" if obj.respond_to?(:updated_at)}
                ...
                seed_row << "#{var_name} = #{klass_name}.create(#{(fk_vals + data).join(', ')})\n"
                unless is_airtable
              ...
              updates.each { |update| seed_row << update } # Anything that needs patching up after-the-fact
              # More than 500K rows?
              # ... then break it into multiple files
              seeds_file.write(seed_row)
              if (seeds_len += seed_row.length) > 48000000 # Under 50 meg files each
                # seeds_file.write("ActiveRecord::Base.connection.execute('COMMIT;')\n")
                seeds_file.close
                ...
                # seeds_file.write("ActiveRecord::Base.connection.execute('BEGIN TRANSACTION;')\n")
                seeds_len = 0
              end
              seed_row = +''
            end
        ...
        airtable_assoc_recids.each do |table_name, assoc_pairs| # N:M links
          ...
          seed_row << "  if ActiveRecord::Migration.table_exists?('#{table_name}')\n"
          seed_row << "    # Duplicate data found in: #{dupes.keys.join(', ')}\n" unless dupes.empty?
          seed_row << "    puts 'Seeding Airtable associations for #{table_name.camelize.singularize}'\n"
          assoc_pairs.each do |_k2, pair_values|
            seed_row << "#{table_name.singularize.camelize}.create(#{pair_values})\n"
          end
          seed_row << "  end\n"
          airtable_assocs_done[table_name] = assoc_pairs
        end
        seeds_file.write(seed_row) unless seed_row.blank?
